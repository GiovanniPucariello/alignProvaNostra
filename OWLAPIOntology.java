...
package src.main.java.fr.inrialpes.exmo.ontowrap.owlapi10;
...
import src.main.java.fr.inrialpes.exmo.ontowrap.Annotation;
import src.main.java.fr.inrialpes.exmo.ontowrap.OntologyFactory;
import src.main.java.fr.inrialpes.exmo.ontowrap.HeavyLoadedOntology;
import src.main.java.fr.inrialpes.exmo.ontowrap.BasicOntology;
import src.main.java.fr.inrialpes.exmo.ontowrap.OntowrapException;
import src.main.java.fr.inrialpes.exmo.ontowrap.util.EntityFilter;
...
/**
 * Store the information regarding ontologies in a specific structure Acts as an
 * interface with regard to an ontology APY
 */
...
public class OWLAPIOntology extends BasicOntology<OWLOntology> implements HeavyLoadedOntology<OWLOntology> {
	...
	public Object getEntity(URI uri) throws OntowrapException {
		try {
			...
			if (result == null)
				result = onto.getDataProperty(uri);
			if (result == null)
				result = onto.getObjectProperty(uri);
			if (result == null)
				result = onto.getIndividual(uri);
			return result;
	...
	protected Set<String> getAnnotations(final OWLEntity e, final String lang, final String typeAnnot)
			throws OWLException {
		...
		// OLD IMPLEMENTATION
		/*
		 * Set<String> annots = new HashSet<String>(); for (Object objAnnot :
		 * e.getAnnotations(onto)) { OWLAnnotationInstance annot =
		 * (OWLAnnotationInstance) objAnnot; String annotUri =
		 * annot.getProperty().getURI().toString(); if (annotUri.equals(typeAnnot) ||
		 * typeAnnot==null) { if ( annot.getContent() instanceof OWLDataValue && (
	...
	// ***JE:
	// We should solve this issue, is it better to go this way or tu use the OWL
	// API?
	...
	protected Set<?> getEntities(Class<? extends OWLEntity> c) throws OWLException {
		...
		/*
		 * Set<Object> entities = new HashSet<Object>(); for (Object obj :
		 * ec.entities()) { // JD: OWLEntitytCollector seems to return anonymous
		 * entities :&& ((OWLEntity)obj).getURI()!=null if (c.isInstance(obj) &&
		 * ((OWLEntity) obj).getURI()!=null) { // &&((OWLEntity)
		 * obj).getURI().toString().startsWith(onto.getURI().toString()) ){
		 * entities.add(obj); } } return entities;
		 */
	}
	...
	public Set<Object> getSubClasses(Object cl, int local, int asserted, int named) {
		...
		for (Object c : getClasses()) {
			if (getSuperClasses((OWLClass) c, local, asserted, named).contains(cl))
				sbcl.add(c);
		}
		return sbcl;
	}
	...
	/*
	 * In the OWL API, there is no properties: there are SuperClasses which are
	 * restrictions
	 */
	...
	public Set<Object> getInstances(Object cl, int local, int asserted, int named) {
		...
		try {
			for (Object i : getIndividuals()) {
				// if ( getClasses( (OWLIndividual)i, local, asserted, named ).contains( cl ) )
				// sbcl.add( i );
				if (((OWLIndividual) i).getTypes(getOntology()).contains(cl))
					sbcl.add(i);
			}
		} catch (OWLException ex) {
		 	log.error("error");
	...
	public Set<Object> getSubProperties(Object pr, int local, int asserted, int named) {
		...
		for (Object p : getProperties()) {
			if (getSuperProperties((OWLProperty) p, local, asserted, named).contains(pr))
				sbpr.add(p);
		}
		return sbpr;
	}
